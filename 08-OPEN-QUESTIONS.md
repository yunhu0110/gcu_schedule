...
- **ADR-014** · 2026-07-28 · **아이폰 1명은 PWA(웹앱), 안드로이드 5명은 기존 APK 유지.** 웹 빌드(`expo export -p web`)를 GitHub Pages(`https://yunhu0110.github.io/gcu_schedule/`)에 올리고, 사파리 "홈 화면에 추가"로 쓴다. · 실제 아이폰 사용자가 1명이라 Apple Developer Program $99/년 + 90일마다 재빌드가 비용 대비 맞지 않음. 무료·무만료. · 버린 대안: TestFlight(유료, 보류이며 폐기는 아님), 애드혹(유료 멤버십 동일 필요), 6명 전원 PWA 통일(APK가 애니메이션·OTA·푸시에서 우위라 다운그레이드).
  ...
  - **브랜치 기준(2026-07-29 정정)**: `feat/pwa-web`은 `main`이 아니라 **`claude/calendar-voting-ui-updates-u6ewha` 위에** 올라가 있다. 폰에 실제로 깔린 OTA(preview 채널 최신)가 그 브랜치라, main 기준으로 빌드했더니 웹 달력만 구버전(SVG 코발트→네온 그라데이션)으로 나왔다. **웹은 앱과 같은 커밋에서 빌드해야 한다.**
  - **후속 결정 필요**: (1) `main`과 `claude/calendar-voting-ui-updates-u6ewha`가 양방향으로 갈라져 있다 — main에만 있는 `홈 스와이프·낙서장 재배치`(ae306f5)·`저작권 푸터`(94e5d37)는 폰에 없다. 어느 쪽으로 통일할지 결정 필요. (2) 통일 후 `deploy-web.yml` 트리거를 `main`으로 바꿀지 결정할 것.
