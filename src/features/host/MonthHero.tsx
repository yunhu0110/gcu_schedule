 ...
 * 넘기는 동안 옆 카드는 살짝 작아지고 흐려진다(스크롤 위치 보간).
...
type Props = {
  ...
  canFix: boolean; // 그 달 모임장만 날짜 확정·변경
  onOpenCalendar: () => void;
  onEditDate: () => void;
};

export function MonthHero({ month, width, pageIndex, scrollX, confirmed, canFix, onOpenCalendar, onEditDate }: Props) {
  ...
  return (
    <Animated.View style={[styles.page, { width, opacity, transform: [{ scale }] }]}>
